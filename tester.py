...
def send(lat, lon, altitude, course, speed, battery, alarm, ignition, accuracy, rpm, fuel, driverUniqueId):
    try:
        ...
        if fuel:
            params += (('fuel', fuel),)
        if driverUniqueId:
            params += (('driverUniqueId', driverUniqueId),)

        conn.request('GET', '?' + urllib.parse.urlencode(params))
        response = conn.getresponse()
        response_data = response.read()
        print(f'Response Status: {response.status}, Response Data: {response_data}')
...
while True:
    ...
    send(lat1, lon1, altitude, course(lat1, lon1, lat2, lon2), speed, battery, alarm, ignition, accuracy, rpm, fuel, driverUniqueId)
    time.sleep(period)
    index += 1
